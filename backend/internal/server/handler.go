...
import (
	...
	"github.com/example/blindbox-backend/internal/logging"
...
func NewHandler(cfg *config.AppConfig) *Handler {
	...
	smsProvider := sms.NewProvider(cfg.SMS)
	svc := services.NewServiceLayer(repo, cfg, limiter, smsProvider, logger)
